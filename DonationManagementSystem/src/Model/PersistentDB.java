    ...
    public void truncateDB() throws Exception
    {
        ...
        try{
            if(conn != null)
            {
                ...
                String template = "DELETE FROM PLEDGEINFO"; 
                statement = conn.prepareStatement(template);
                statement.executeUpdate();

                template = "DELETE FROM PROPERTYINFO"; 
                statement = conn.prepareStatement(template);
                statement.executeUpdate();

                template = "DELETE FROM DONATION"; 
                statement = conn.prepareStatement(template);
                statement.executeUpdate();

                template = "DELETE FROM VOLUNTEER"; 
                statement = conn.prepareStatement(template);
                statement.executeUpdate();

                template = "DELETE FROM PROJECT"; 
                statement = conn.prepareStatement(template);
                statement.executeUpdate();


                template = "DELETE FROM DONOR"; 
                statement = conn.prepareStatement(template);
                statement.executeUpdate();

                template = "DELETE FROM ADMIN"; 
                statement = conn.prepareStatement(template);
                statement.executeUpdate();
                
                template = "DELETE FROM ORGANIZATION"; 
                statement = conn.prepareStatement(template);
                statement.executeUpdate();
...
//            type = rSet.getString("Type");
            ...
            if (projId != 0) { //Update corressponding Project 
